...
# POST Artigo
@router.post('/', status_code=status.HTTP_201_CREATED, response_model=ArtigoShema)
...
@router.get('/', response_model=List[ArtigoShema])
...
@router.get('/{artigo_id}', response_model=ArtigoShema, status_code=status.HTTP_200_OK)
async def get_artigo_id(artigo_id:int, db:AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        if artigo:
            ...
            raise HTTPException(
                detail='Artigo não encontado.',
                status_code=status.HTTP_404_NOT_FOUND
            )


@router.put('/{artigo_id}', response_model=ArtigoShema, status_code=status.HTTP_202_ACCEPTED)
async def put_artigo_id(
    artigo_id:int, 
    artigo: ArtigoShema, 
    db: AsyncSession = Depends(get_session), 
    usuario_logado: UsuarioModel = Depends(get_current_user)
